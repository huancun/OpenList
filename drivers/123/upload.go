...
func (d *Pan123) newUpload(ctx context.Context, upReq *UploadResp, file model.FileStreamer, up driver.UpdateProgress) error {
	...
	ss, err := stream.NewStreamSectionReader(file, int(chunkSize), 1)
	if err != nil {
		return err
	}
	for i := 1; i <= chunkCount; i += batchSize {
		...
		for j := start; j < end; j++ {
			...
			reader, err := ss.GetSectionReader(chunkSize*int64(j-1), curSize, j)
			if err != nil {
				return err
			}
...
func (d *Pan123) uploadS3Chunk(ctx context.Context, upReq *UploadResp, s3PreSignedUrls *S3PreSignedURLs, cur, end int, reader io.ReadSeeker, curSize int64, retry bool, getS3UploadUrl func(ctx context.Context, upReq *UploadResp, start int, end int) (*S3PreSignedURLs, error)) error {
